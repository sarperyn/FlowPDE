...
tests/         139 tests (132 fast, 7 marked slow)
notebooks/     Analysis and visualization
experiments/   Runnable ablations (exp01-05) + report_analysis/, report_figures/
...
- **The finite-ensemble correction has a direction, and it was wrong.** `spread_skill_ratio` multiplied where it should have divided: a K-member ensemble *under*-estimates spread, so the raw ratio sits at sqrt(K/(K+1)) < 1 when calibrated and the correction must scale it *up*. The bug survived because the only test used K=50, where the two directions differ by 2% and the tolerance was 0.1. `test_finite_ensemble_correction_points_the_right_way` now pins it at K=2,4,8,32. Any metric with a small-sample correction needs a small-sample test.
- **The Poisson dataset defaults to the "easy" variant** — 3 sine terms at wavenumbers 1–3 (`source_num_terms`, `source_max_mode`), a nine-dimensional subspace. exp10 measured the cost of that choice: widening both to 8 moves ConvNet relative L2 from 0.054 to 0.078. Report the hard regime, or say which one a number came from.

## Known open items

- **`configs/benchmark_configs.py` does not import.** It passes `f_cutoff_min`, `f_cutoff_max`, `f_gaussian_bump_prob` and `f_gaussian_bump_max_bumps` to `DarcyConfig`, which accepts only `f_cutoff`, `f_amplitude_min`, `f_amplitude_max`. Nothing imports it, which is why this went unnoticed.
- **`experiments/exp05_inverse_conditioning_ablation/` was reconstructed after the fact.** Its runner was not committed alongside the run; it has since been rebuilt from the resolved configs, so the configuration is reproducible but the weights in `results/` were produced by code that no longer exists in that exact form. One cell (κ-only null at 5000) saved no checkpoint.
...
- **`flow._target_dim` is set as a side effect of `compute_loss`.** A checkpoint loaded fresh for inference never calls it, so `sample()` silently falls back to the condition dimension. Pass `target_shape=` explicitly when sampling from a restored checkpoint (`experiments/exp12_mcmc_reference/compare.py` has to). Set the target shape at construction instead.
- ~~Docstring escape warnings in `datasets/exponax/{burgers,darcy}.py`~~ — fixed; both files now compile clean under `-W error::SyntaxWarning`.
